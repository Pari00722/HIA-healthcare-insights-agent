...
```bash
git clone https://github.com/pari/hia.git
cd hia
```
...
If you do not configure `.streamlit/secrets.toml`, the app now starts in a local demo mode automatically:

- Local in-memory demo user and session history
- Sample-report analysis without Groq
- Basic follow-up chat from the report text

To enable the full production flow, copy `.streamlit/secrets.toml.example` to `.streamlit/secrets.toml` and add your real Supabase and Groq credentials.
