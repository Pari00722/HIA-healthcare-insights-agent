...
class AuthService:
    def __init__(self):
        try:
            ...
            raise RuntimeError(
                "Missing Supabase configuration. Add SUPABASE_URL and SUPABASE_KEY to .streamlit/secrets.toml."
            ) from exc
    ...
    def sign_in(self, email, password):
        try:
            ...
            if auth_response and auth_response.user:
                ...
                if not user_data:
                    # Auto-create user row if missing (e.g. RLS blocked insert on signup)
                    try:
                        user_meta = auth_response.user.user_metadata or {}
                        user_data = {
                            "id": auth_response.user.id,
                            "email": auth_response.user.email,
                            "name": user_meta.get("name", auth_response.user.email),
                            "created_at": datetime.now().isoformat(),
                        }
                        self.supabase.table("users").insert(user_data).execute()
                    except Exception:
