...
class SessionManager:
    ...
    def ensure_auth_service():
        """Return an initialized auth service for the current runtime mode."""
        ...
        if auth_service is None:
            if SessionManager.is_demo_mode():
                ...
                st.session_state["auth_service"] = LocalAuthService()
            else:
                from auth.auth_service import AuthService

                st.session_state["auth_service"] = AuthService()

        return st.session_state.get("auth_service")

    @staticmethod
    def get_missing_secrets():
        """Return any required Streamlit secrets that are not configured."""
        try:
            return [key for key in SessionManager.REQUIRED_SECRETS if key not in st.secrets]
        except Exception:
            return list(SessionManager.REQUIRED_SECRETS)
    ...
    def init_session():
        ...
        missing_secrets = SessionManager.get_missing_secrets()
        st.session_state.missing_secrets = missing_secrets
        st.session_state.demo_mode = bool(missing_secrets)
        st.session_state.configuration_ready = True
        st.session_state.integration_ready = not missing_secrets

        if SessionManager.is_demo_mode():
            auth_service = st.session_state.get("auth_service")
            if (
                auth_service is None
                or auth_service.__class__.__name__ != "LocalAuthService"
            ):
                from auth.local_auth_service import LocalAuthService

                st.session_state["auth_service"] = LocalAuthService()
            else:
                auth_service.ensure_demo_session()
        else:
            auth_service = st.session_state.get("auth_service")
            if (
                auth_service is None
                or auth_service.__class__.__name__ != "AuthService"
            ):
                from auth.auth_service import AuthService
                st.session_state["auth_service"] = AuthService()
        ...
        if 'user' in st.session_state:
            user_data = SessionManager.ensure_auth_service().validate_session_token()
            if not user_data:
                SessionManager.clear_session_state()
    ...
    def create_chat_session():
        ...
        return SessionManager.ensure_auth_service().create_session(
    ...
    def get_user_sessions():
        ...
        if not SessionManager.is_authenticated():
            return False, []
        return SessionManager.ensure_auth_service().get_user_sessions(
            st.session_state.user['id']
        )
    ...
    def delete_session(session_id):
        ...
        if not SessionManager.is_authenticated():
            return False, "Not authenticated"
        return SessionManager.ensure_auth_service().delete_session(session_id)
    ...
    def login(email, password):
        """Handle user login."""
        auth_service = SessionManager.ensure_auth_service()
        success, user_data = auth_service.sign_in(email, password)
    ...
    def sign_up(email, password, name):
        """Handle user signup."""
        auth_service = SessionManager.ensure_auth_service()
        success, user_data = auth_service.sign_up(email, password, name)
