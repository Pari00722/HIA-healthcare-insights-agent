...
def show_configuration_screen():
    missing_secrets = st.session_state.get("missing_secrets", [])
    missing_keys = "\n".join(f"- {key}" for key in missing_secrets)

    st.markdown(
        f"""
        <div style='text-align: center; padding: 40px 20px;'>
            <h1>{APP_ICON} {APP_NAME}</h1>
            <h3>Finish local setup to start the app</h3>
            <p style='font-size: 1.05em; color: #666;'>{APP_TAGLINE}</p>
        </div>
        """,
        unsafe_allow_html=True,
    )
    ...
    st.warning(
        "Missing required Streamlit secrets. Add the keys below to `.streamlit/secrets.toml` and restart the app."
    )
    st.code(
    ...
    if missing_keys:
        st.info(f"Missing keys:\n{missing_keys}")

    st.markdown(
        "This file is already gitignored, so your local credentials will stay out of version control."
    )
...
def show_runtime_notice():
    if st.session_state.get("demo_mode"):
        st.info(
            "Running in local demo mode. Sessions and analysis work locally in memory; add `.streamlit/secrets.toml` to enable Supabase auth and Groq-backed responses."
        )
...
def main():
    ...
    if st.session_state.get("demo_mode"):
        with st.expander("Enable full integrations", expanded=False):
